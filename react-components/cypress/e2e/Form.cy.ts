describe('Form E2E', () => {
  ...
  it('after correct filling form card should be added', () => {
    ...
    cy.get('.submit-button').click();
    cy.get('.user-card-container').should(
      'have.text',
      'Alina LaniukUser birthday: 1999-12-31User gender: femaleUser favorite desserts: cakeUser favorite additives:chocolate caramel nuts berries vanilla '
    );
    cy.get('input[id="text-input"]').should('have.value', '');
  });
});
